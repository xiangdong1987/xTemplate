...
(function ($) {
    function Plot(placeholder, data_, options_, plugins) {
        ...
        var myRetinaMdo = (window.devicePixelRatio > 1),
            ...
            ctx = null, octx = null,
            xaxes = [], yaxes = [], xgridpos = [], xgridHL = -100,
        ...
        function numFormat(val) {
            if (val > 1024 * 1024 * 1024) return (Math.round(val * 100 / 1024 / 1024 / 1024) / 100) + 'G';
            else if (val > 1024 * 1024) return (Math.round(val * 100 / 1024 / 1024) / 100) + 'M';
            else if (val > 1024) return (Math.round(val * 100 / 1024) / 100) + 'K';
            return val;
        }
        ...
        function drawGrid() {
            ...
            for (var j = 0; j < axes.length; ++j) {
                ...
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    ctx.beginPath();
                    if (axis.direction == "x" && (i == xgridHL || i == xgridHL - 1))
        ...
        function showLine(i) {
            series[i].show = (series[i].show === false) ? true : false;
            setData(this.getData());
            setupGrid();
            draw();
            return series[i].show === false ? "#DDDDDD" : "#545454";
        }
        ...
        function insertLegend() {
            ...
            for (var i = 0; i < series.length; ++i) {
                ...
                fragments.push(
                    ...
                    '<td class="legendLabel"><span style="cursor:pointer;color:' + (s.show === false ? "#DDDDDD" : "#545454") + '" onclick="this.style.color=$(\'#'+placeholder.prop('id')+'\').data(\'plot\').showLine(' + i + ')">' + label + '</span></td>');
            }
            if (rowStarted)
                fragments.push('</tr>');
        ...
        function triggerClickHoverEvent(eventname, event, seriesFilter) {
            ...
            /*for(i=0;i<xgridpos.length;i++){
             if(canvasX < xgridpos[i]){
             break;
             }
             }
             if(i!=xgridHL){
